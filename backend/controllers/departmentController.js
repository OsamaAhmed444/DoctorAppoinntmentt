...
export const getDepartmentsCount = async (req,res,next) => {
  try {
  ...
  try {
    ...
    if (name !== undefined) {
      if (
        ...
        throw createError("Department name cannot be empty",400);
      }
      ...
      if (normalizedName !== department.name) {
        ...
        if (existingDepartment) {
          throw createError("Department name already exists",400);
        }
      }
    ...
    if (description !== undefined) {
      if (typeof description !== "string") {
        throw createError("Description must be valid text",400);
      }
    ...
    if (
      req.file && !error.statusCode && error.code !== 11000 ) {
      await deleteImageFile(req.file.filename);
    }
    next(error);
  }
...
export const deleteDepartment = async (req, res, next) => {
  try {
    ...
    if (doctorsCount > 0) {
      throw createError(
        "Cannot delete department because it has doctors assigned to it",400);
    }
